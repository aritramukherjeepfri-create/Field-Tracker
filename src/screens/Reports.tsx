...
export function Reports() {
  ...
  const totals = useMemo(() => {
    const budgetTotal = scopedProjects.reduce((s, p) => s + p.budgetTotal, 0);
    const budgetSpent = scopedProjects.reduce((s, p) => s + p.budgetSpent, 0);
  ...
  function exportCSV() {
    downloadCSV('field_tracker_summary.csv', [
      ...
      ...scopedProjects.map((p) => [
        ...
        p.budgetTotal,
        p.budgetSpent,
        p.budgetTotal > 0 ? Math.round((p.budgetSpent / p.budgetTotal) * 100) : 0,
        p.team.length,
        p.logs.length,
        p.expenses.length,
      ]),
        ...
        <tbody>
          ${scopedProjects.map((p) => `
            <tr>
              <td>${p.name}</td>
              <td>${p.status}</td>
              <td>${formatCurrency(p.budgetTotal, p.currency)}</td>
              <td>${formatCurrency(p.budgetSpent, p.currency)}</td>
              <td>${p.budgetTotal > 0 ? Math.round((p.budgetSpent / p.budgetTotal) * 100) : 0}%</td>
              <td>${p.team.length}</td>
              <td>${p.logs.length}</td>
    ...
    <div className="px-md py-lg flex flex-col gap-xl pb-2xl">
      ...
      <section className="flex flex-col gap-sm">
        ...
        <div className="flex flex-col gap-sm">
          {scopedProjects.map((p) => {
            const pct = p.budgetTotal > 0 ? Math.round((p.budgetSpent / p.budgetTotal) * 100) : 0;
            return (
              <div key={p.id} className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md flex flex-col gap-xs">
                ...
                <div className="flex flex-wrap gap-md text-[11px] text-on-surface-variant">
                  <span>{formatCurrency(p.budgetSpent, p.currency)} / {formatCurrency(p.budgetTotal, p.currency)}</span>
