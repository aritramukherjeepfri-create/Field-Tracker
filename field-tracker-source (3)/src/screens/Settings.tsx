...
export function Settings() {
  ...
  const { orgMembers, canManage, refresh, updateMemberRole, updateMemberFinancialAccess } = useApp();
  ...
  return (
    <div className="px-md py-lg flex flex-col gap-xl pb-2xl">
      ...
      <section className="flex flex-col gap-sm">
        ...
        <p className="text-body-sm text-on-surface-variant">
          "Financials" controls whether someone can see budget totals and how much has been spent. Everyone can still log expenses and field notes regardless of this setting.
        </p>
        <div className="flex flex-col gap-xs">
          {orgMembers.map((m) => (
            <div key={m.id} className="rounded-xl bg-surface-container-low px-md py-sm flex flex-col gap-xs">
              ...
              {canManage && m.id !== profile?.id && (
                <label className="flex items-center gap-xs text-[12px] text-on-surface-variant pl-[44px]">
                  <input
                    type="checkbox"
                    checked={m.canViewFinancials}
                    onChange={(e) => updateMemberFinancialAccess(m.id, e.target.checked)}
                    className="accent-primary"
                  />
                  Can view budget totals &amp; spend
                </label>
              )}
              {!canManage || m.id === profile?.id ? (
                <p className="text-[11px] text-on-surface-variant pl-[44px]">
                  {m.canViewFinancials ? 'Can view budget totals & spend' : 'Financials hidden'}
                </p>
              ) : null}
            </div>
          ))}
